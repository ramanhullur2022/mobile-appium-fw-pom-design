...
public class TC_SendMsg_Sporsmal_Reply_JA_TC006 extends Base_Class {
	 ...
	 * Test Case ID : TC_SendMsg_Sporsmal_Reply_JA_TC006
	...
	public void sendAndVerifySporsmalReply() throws InterruptedException, Exception {
		...
		String statusLog = "TC_SendMsg_Sporsmal_Reply_JA_TC006";
		try {

			// start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendMsg_Sporsmal_Reply_JA_TC006",
					"Send new message from Teacher to the Student with the Spørsmål enabled and Student responds as JA then check the response received by teacher");
...
//			+ " and selected Eleven + Foresatte checkbox");
		...
		try {
			...
			// JA Reply
			statusLog = "Clicking on JA and sending";
			page2.clickOnJAAndSendMsg();
			test.log(Status.PASS, "Clicked on JA then clicked in Send button");
			...
			if (expected_result.equals(actual_result)) {
				...
				test.log(Status.PASS,
						"Sent new message from Teacher to the Student with the Spørsmål enabled and Student responds as JA then checked the response received by teacher successfully");
				...
				test.log(Status.FAIL,
						"Sending new message from Teacher to the Student with the Spørsmål enabled and Student responds as JA then checking the response received by teacher is failed");
			...
			page3.getActualResultJA(act_text);
