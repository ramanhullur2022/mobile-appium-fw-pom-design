...
public class TC_SendReceive_Attachment_TC003 extends Base_Class{
	 ...
	 * Test Case ID  : TC_SendReceive_SendAttachment_TC003
	 * Test Summary  : Teacher sending a PDF attachment to the Student with the message and the Student receive and opening the attachment.
	...
	public void sendMultipleAttachmentsWithMsg() throws InterruptedException, Exception {
		String statusLog = "TC_SendReceive_SendAttachment_TC003";
		
		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendReceive_Attachment_TC003", "Teacher sending a PDF attachment to the Student with the message and the Student receive the attachment and opening it");
			...
			logger.info("PDF file found from search results");
			test.log(Status.PASS, "PDF file found in search results");
			
			String sc_attachemnt = Base_Class.captureScreenExtent(driver2, "attachement_file");
			test.pass("PDF attachment file found", MediaEntityBuilder.createScreenCaptureFromPath(sc_attachemnt).build());
			
			statusLog = "Attachment of PDF file selecting";
			page.clickOnFileAndAttach();
			logger.info("Attachment file selected and attached");
			test.log(Status.PASS, "PDF attachment file selected and attached with the message");
			
			String file_attached = Base_Class.captureScreenExtent(driver2, "pdf_file_attached");
			test.pass("PDF file attached with the message", MediaEntityBuilder.createScreenCaptureFromPath(file_attached).build());
...
//			test.log(Status.PASS, "Clicked on Searched Student");
		...
		try {
			...
			statusLog = "Clicking and opening the PDf attachment in the message";
			page2.clickOnAttachmentInMessageAndOpen();
			logger.info("Clicked and opened the PDf attachment in the message");
			test.log(Status.PASS, "Clicked and opened the PDf attachment in the message sent by Teacher("+config.get_teacher_username()+")");
			
			String pdffile_attached = Base_Class.captureScreenExtent(driver2, "pdf_file_opened");
			test.pass("PDF file opened from received the message", MediaEntityBuilder.createScreenCaptureFromPath(pdffile_attached).build());
		
			statusLog = "Closing the PDF file";
			...
			//if (expected_result != actual_result) {
				...
				test.pass("Verification Completed",
						...
						build());
				test.log(Status.PASS, "Teacher sent a PDF attachment to the Student with the message and the Student verified the message successfully");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Teacher couldn't able send a PDF attachment to the Student with message and the Student verification failed");
