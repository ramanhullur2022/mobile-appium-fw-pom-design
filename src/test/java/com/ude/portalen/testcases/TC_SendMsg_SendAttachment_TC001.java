...
public class TC_SendMsg_SendAttachment_TC001 extends Base_Class{
	 ...
	 * Test Summary  : Teacher send a message to the Student with an attachment(Vedlegg)
	...
	public void disableMessageByEmail() throws InterruptedException, Exception {
		...
		try {
			...
			test = extent.createTest("TC_SendReceive_SendAttachment_TC001", "Teacher send a message to the Student with an attachment(Vedlegg)");
		...
		try {
			...
			//if (expected_result != actual_result) {
				...
				test.pass("Verification Completed",
						...
						build());
				test.log(Status.PASS, "Teacher sent a message to the Student with an attachment(Vedlegg) successfully");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Teacher sent a message to the Student with an attachment(Vedlegg) verifications failed");
