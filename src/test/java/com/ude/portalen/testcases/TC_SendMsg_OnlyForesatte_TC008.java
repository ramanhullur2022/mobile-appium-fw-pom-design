...
import com.ude.portalen.pageobject.Page_SendMsg_OnlyEleven_TC007;
...
public class TC_SendMsg_OnlyForesatte_TC008 extends Base_Class{
	...
	/*
	 * Test Suite ID : UDEMA_SendReceive 
	 * Test Case ID  : TC_SendMsg_OnlyForesatte_TC008
	 * Test Summary  : Send a message from Teacher to the Student with Only Eleven option
	...
	public void send_Receive_New_Message_with_Svar_Av() throws InterruptedException, Exception {
		String statusLog = "TC_SendMsg_OnlyForesatte_TC008";
		
		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendMsg_OnlyForesatte_TC008", "Send a message from Teacher to the Student with Only Foresatte option");
			...
			Page_SendMsg_OnlyEleven_TC007 page = new Page_SendMsg_OnlyEleven_TC007(driver2);
...
//			page.clickOnSearchResultOfStudentOnlyForesatte();
...
//			test.log(Status.PASS, "Clicked on student "+config.get_student_username()+" and selected Eleven + Foresatte checkbox");
			...
			searchForUserAndSelect(driver2, "student", "test", "foresatte_only");
			...
			String screenshot08 = Base_Class.captureScreenExtent(driver2, "received_email");
			test.pass("Receipient selected with only foresatte",MediaEntityBuilder.createScreenCaptureFromPath(screenshot08).build());
		...
		try {
			...
			Page_SendMsg_OnlyEleven_TC007 page2 = new Page_SendMsg_OnlyEleven_TC007(driver);
			...
			expected_result = page2.getExpectedResultFat(exp_text);
			...
			if (expected_result.equals(actual_result)) {
				captureScreen(driver, "received_email");
				String screenShotPath = Base_Class.captureScreenExtent(driver, "received_email");
				test.pass("Verification Completed",
						MediaEntityBuilder.
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.PASS, "Sent a message from Teacher to the Student with only Foresatte option is successful");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Sending a message from Teacher to the Student with only Foresatte option is failed at verification");
