...
import com.ude.portalen.pageobject.Page_SendMsg_Svar_Av_Sporshmal_TC003;
...
public class TC_SendMsg_Svar_Av_Sporsmal_TC003 extends Base_Class{
	 ...
	 * Test Case ID  : TC_SendMsg_Svar_Av_Sporsmal_TC003
	 * Test Summary  : Send new message from Teacher to the Student with the Spørsmål enabled and Svar av optioned is enabled
	...
	public void send_Receive_New_Message_with_Spørsmål() throws InterruptedException, Exception {
		...
		String statusLog = "TC_SendMsg_Svar_Av_Sporsmal_TC003";
		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendMsg_Svar_Av_Sporsmal_TC003", "Send new message from Teacher to the Student with the Spørsmål enabled and Svar av optioned is enabled");
			...
			Page_SendMsg_Svar_Av_Sporshmal_TC003 page = new Page_SendMsg_Svar_Av_Sporshmal_TC003(driver2);
			...
			statusLog = "Verifying isReplyAllowed button and switching to Svar av";
			page.clickSvarAvButton();
			test.log(Status.PASS, "Svar av is enabled");
			
			String screenShot2 = Base_Class.captureScreenExtent(driver2, "svar_av");
			test.pass("Svar av enabled", MediaEntityBuilder.createScreenCaptureFromPath(screenShot2).build());
...
//			test.log(Status.PASS, "Clicked on student "+config.get_student_username()+" and selected Eleven + Foresatte checkbox");
		...
		try {
			...
			Page_SendMsg_Svar_Av_Sporshmal_TC003 page2 = new Page_SendMsg_Svar_Av_Sporshmal_TC003(driver2);
			...
			test.log(Status.PASS, "Sporsmal Email matching with the mail sent by Teacher("+config.get_teacher_username()+")");
			...
			if (expected_result.equals(actual_result)) {
				...
				test.pass("Verification Completed",
						...
						build());
				test.log(Status.PASS, "Sending new message from Teacher to the Student with the Spørsmål enabled and Svar av optioned is enabled is successful");
				...
				test.log(Status.FAIL, "Sending new message from Teacher to the Student with the Spørsmål enabled and Svar av optioned is enabled is failed in verification");
