...
public class TC_SendMsg_OnlyEleven_TC007 extends Base_Class{
	 ...
	 * Test Case ID  : TC_SendMsg_OnlyEleven_TC007
	...
	public void send_Receive_New_Message_with_Svar_Av() throws InterruptedException, Exception {
		String statusLog = "TC_SendMsg_OnlyEleven_TC007";
		
		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendMsg_OnlyEleven_TC007", "Send a message from Teacher to the Student with Only Eleven option");
...
//			test.log(Status.PASS, "Clicked on student "+config.get_student_username()+" and selected Eleven + Foresatte checkbox");
		...
		try {
			...
			Page_SendMsg_OnlyEleven_TC007 page2 = new Page_SendMsg_OnlyEleven_TC007(driver2);
			...
			if (expected_result.equals(actual_result)) {
				...
				test.pass("Verification Completed",
						...
						build());
				test.log(Status.PASS, "Sent a message from Teacher to the Student with only Eleven option is successful");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Sending a message from Teacher to the Student with only Eleven option is failed at verification");
