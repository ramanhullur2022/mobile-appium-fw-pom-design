...
public class TC_SendReceive_Attachment_TC002 extends Base_Class{
	 ...
	 * Test Case ID  : TC_SendReceive_SendAttachment_TC002
	 * Test Summary  : Teacher sending a message to the Student with multiple attachments(>1)
	...
	public void sendMultipleAttachmentsWithMsg() throws InterruptedException, Exception {
		String statusLog = "TC_SendReceive_SendAttachment_TC002";
		
		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendReceive_Attachment_TC002", "Teacher sending a message to the Student with multiple attachments(>1)");
			...
			test.pass("First file attached with the message", MediaEntityBuilder.createScreenCaptureFromPath(file_attached).build());
			...
			String legg_till_vedlegg = Base_Class.captureScreenExtent(driver2, "Legg_til_vedlegg2");
			...
			String sc_attachemnt_2 = Base_Class.captureScreenExtent(driver2, "attachement_file2");
			test.pass("Attachment 2 file found", MediaEntityBuilder.createScreenCaptureFromPath(sc_attachemnt_2).build());
			...
			test.pass("Second file attached with the message", MediaEntityBuilder.createScreenCaptureFromPath(file_attached3).build());
		...
		try {
			...
			//if (expected_result != actual_result) {
				...
				test.pass("Verification Completed",
						...
						build());
				test.log(Status.PASS, "Teacher sending a message to the Student with multiple attachments(>1) is successful");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Teacher couldn't able send a message to the Student with multiple attachments(>1).");
