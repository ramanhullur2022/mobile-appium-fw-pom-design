...
import com.ude.portalen.pageobject.Page_SendMsg_TwoStudents_TC009;
...
public class TC_SendMsg_TwoStudents_TC009 extends Base_Class{
	...
	/*
	 * Test Suite ID : SendReceive_New
	 * Test Case ID  : TC_SendMsg_TwoStudents_TC009 
	 * Test Summary  : Send a message from Teacher to multiple Students/Contacts
	 * Test Author   : Ramana Gouda
	 */

	//@Test(dataProvider="testData")
	@Test
	public void sendAMessageToMultipleStudents() throws InterruptedException, Exception {
		String statusLog = "TC_SendMsg_TwoStudents_TC009";

		try {

			//start_Server();
			extent = ExtentManager.GetExtent();
			test = extent.createTest("TC_SendMsg_TwoStudents_TC009", "Send a message from Teacher to multiple Students/Contacts");
			...
			Page_SendMsg_TwoStudents_TC009 page = new Page_SendMsg_TwoStudents_TC009(driver2);
...
//			test.log(Status.PASS, "Clicked on student "+config.get_student_username()+" and selected Eleven + Foresatte checkbox");
			...
			//second student contact selection

			statusLog = "Again clicking Till(To) Button for second student contact";
			page.clickOnTillButton();
			logger.info("Till Button clicked");
			test.log(Status.PASS, "Clicked on Till(to) button to select the receipent student name from contact list");
...
//			statusLog = "Clicking on Search tab to find student contact and selecting";
//			page.click_Search_Input_editText_testcase3();
//			test.log(Status.PASS, "Searched student tttta006 in search box");

//			page.click_Search_Result_Student2();
//			logger.info("Clicked on Searched Student");
//			test.log(Status.PASS, "Clicked on Searched Student");

			searchForUserAndSelect(driver2, "student_two", "test", "eleven_and_foresatte");
			logger.info("Clicked on Searched Student");
			test.log(Status.PASS, "Clicked on Searched Student");
			String screenshot_09 = Base_Class.captureScreenExtent(driver2, "student_contact_selected");
			test.pass("Second Student contact selected",MediaEntityBuilder.createScreenCaptureFromPath(screenshot_09).build());
			...
			test.log(Status.FAIL, "Verfication unsuccessfull and Test Case Failed");
		...
		//Student1 verification for received email from teacher
		...
		try {
			...
			Page_SendMsg_TwoStudents_TC009 page2 = new Page_SendMsg_TwoStudents_TC009(driver2);
		...
		//Student2 verification for received email from teacher

		try {
			statusLog = "Logging in as second receipent Student "+config.get_student_username3();
			test.log(Status.PASS, "Logging in to Student("+config.get_student_username3()+") to verify email from Teacher");
			ElevLoginTest elav_login_student = new ElevLoginTest();
			elav_login_student.loginAsStudentForMultipleContacts();
			test.log(Status.PASS, "Login as Student("+config.get_student_username3()+") to verify email from Teacher("+config.get_teacher_username()+") is successful");
			...
			page2.clickOnArkiv();
			logger.info("Clicked on Arkiv then clicked received email by searching it from Arkiv");
			test.log(Status.PASS, "Student("+config.get_student_username3()+") Successfully received email from Teacher("+config.get_teacher_username()+")");
			...
			if (expected_result.equals(actual_result)) {
				...
				test.pass("Verification Completed",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.PASS, "Sent a message from Teacher to multiple Students/Contacts is successful");
				softAssert_TC1.assertEquals(actual_result, expected_result, "Soft Assertion Test Pass");
				logger.info("Verification is done");
				...
				test.fail("Verification failed here!",
						...
						createScreenCaptureFromPath(screenShotPath).
						build());
				test.log(Status.FAIL, "Sending a message from Teacher to multiple Students/Contacts failed at second receipient received mail verification");
				softAssert_TC1.assertNotEquals(actual_result, expected_result, "Soft Assertion Test Fail");
				logger.warn("Verification is done");
			}

			statusLog = "Logging out from Student2";
			page2.logout();
			logger.info("Logged out from Student successfully");
			test.log(Status.PASS, "Logged out from Student("+config.get_student_username3()+")");
